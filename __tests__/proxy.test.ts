...
/**
 * The class test for routing the credentialed drift callback through the
 ...
 *    the origin is routed by its own destination and not by the original URL's;
...
const DEST = 'https://tsm.example.com/api/v1/drift/ingest'
...
const HOP_ROWS: HopRow[] = [
  {
    ...
    env: {},
    redirectTo: 'https://tsm-2.example.com/api/v1/drift/ingest',
    expect: [false, false],
  },
  {
  ...
  },
  {
    ...
    // handing the callback token to a proxy that NO_PROXY says must not see
    // this destination.
    what: 'a redirect INTO NO_PROXY leaves the proxy behind',
    env: { HTTPS_PROXY: PROXY, NO_PROXY: 'internal.example.com' },
    redirectTo: 'https://internal.example.com/api/v1/drift/ingest',
    expect: [true, false],
  },
  {
    // The sharpest row. Resolving once for the original URL would have sent
    // hop 2 direct too — the request the organisation's chokepoint never sees,
    // which is the whole defect this fix exists to close.
    what: 'a redirect OUT OF NO_PROXY picks the proxy up',
    env: { HTTPS_PROXY: PROXY, NO_PROXY: 'tsm.example.com' },
    redirectTo: 'https://elsewhere.example.com/api/v1/drift/ingest',
    expect: [false, true],
  },
  {
    what: 'NO_PROXY covering both ends keeps both hops direct',
    env: { HTTPS_PROXY: PROXY, NO_PROXY: 'tsm.example.com,elsewhere.example.com' },
    redirectTo: 'https://elsewhere.example.com/api/v1/drift/ingest',
    expect: [false, false],
  },
  {
    // http_proxy is not the variable for an https destination; reading it would
    // route a credentialed request through a proxy never nominated for it.
...
describe('the proxy decision is re-taken for every hop', () => {
  it.each(HOP_ROWS)('$what', async (row) => {
    ...
    const response = await postJson(
      ...
      '{"status":"completed"}',
      // Both destinations allowlisted, so the egress decision is not what is
      // being measured here.
      createHostAuthorizer('tsm.example.com,tsm-2.example.com,internal.example.com,elsewhere.example.com'),
  ...
  it('reuses ONE dispatcher per proxy across a chain rather than leaking one per hop', async () => {
    const { impl, calls } = recordingFetch([
      { status: 302, location: 'https://tsm-2.example.com/a' },
      { status: 302, location: 'https://tsm-3.example.com/b' },
      { status: 200 },
    ])
    await postJson(
      DEST,
      {},
      '{}',
      createHostAuthorizer('tsm.example.com,tsm-2.example.com,tsm-3.example.com'),
      { fetchImpl: impl, env: { HTTPS_PROXY: PROXY } },
    )
...
describe('a proxy never widens the egress decision', () => {
  it('refuses an unauthorized destination even when a proxy is configured, without issuing the request', async () => {
    ...
    await expect(
      postJson(DEST, {}, '{}', createHostAuthorizer('allowed.example.com'), {
        fetchImpl: impl,
        env: { HTTPS_PROXY: PROXY },
      }),
    ).rejects.toThrow(/tsm\.example\.com.*not in callback-allowed-hosts/)
  ...
  it('an allowlisted PROXY does not launder an unallowlisted destination', async () => {
    // The bypass this whole shape exists to prevent: the operator permits the
    // proxy host, and a CONNECT tunnel through it to an unauthorized
    // destination is nevertheless still unauthorized egress.
    const { impl, calls } = recordingFetch([{ status: 200 }])
    await expect(
      postJson(DEST, {}, '{}', createHostAuthorizer('proxy.internal'), {
        fetchImpl: impl,
        env: { HTTPS_PROXY: PROXY },
      }),
    ).rejects.toThrow(/tsm\.example\.com.*not in callback-allowed-hosts/)
  ...
  it('refuses a tunnelled redirect hop to an unauthorized host', async () => {
    ...
    await expect(
      postJson(DEST, {}, '{}', createHostAuthorizer('tsm.example.com'), {
        fetchImpl: impl,
        env: { HTTPS_PROXY: PROXY },
      }),
    ).rejects.toThrow(/attacker\.example\.com.*not in callback-allowed-hosts/)
  ...
  it('never asks the authorizer about the proxy — only about destinations', async () => {
    // A structural proof rather than an outcome one: whatever the verdict, the
    // SUBJECT of the decision must always be a destination. If the proxy were
    // ever the subject, an allowlist naming it would start deciding requests
    // that are not addressed to it.
    const asked: string[] = []
    const authorize = createHostAuthorizer('tsm.example.com,tsm-2.example.com')
    const recording = async (host: string) => {
      asked.push(host)
      await authorize(host)
    }
    const { impl } = recordingFetch([
      { status: 302, location: 'https://tsm-2.example.com/api/v1/drift/ingest' },
      { status: 200 },
    ])
    await postJson(DEST, {}, '{}', recording, { fetchImpl: impl, env: { HTTPS_PROXY: PROXY } })
    expect(asked).toEqual(['tsm.example.com', 'tsm-2.example.com'])
  ...
  it('the default-deny arm still refuses a private destination through a proxy', async () => {
    // With no allowlist the private-address refusal is the control, and a proxy
    // must not be a way to reach 169.254.169.254 from a job that could not
    // reach it directly.
    const { impl, calls } = recordingFetch([{ status: 200 }])
    await expect(
      postJson('https://169.254.169.254/latest/meta-data/', {}, '{}', createHostAuthorizer(''), {
        fetchImpl: impl,
        env: { HTTPS_PROXY: PROXY },
      }),
    ).rejects.toThrow(/private, link-local or otherwise/)
    expect(calls).toHaveLength(0)
  })
})
...
describe('an unusable proxy variable fails the step rather than going direct', () => {
  it('refuses, names the variable, never echoes its value, and does not retry', async () => {
    ...
    const { impl, calls } = recordingFetch([{ status: 200 }])
    const error = await postJson(DEST, {}, '{}', createHostAuthorizer('tsm.example.com'), {
      fetchImpl: impl,
      env,
    }).then(
      () => null,
    ...
    expect(error?.message, 'the value can carry a password and must never be echoed').not.toMatch(/not a url/)
...
describe('proxy credentials are registered with the job mask', () => {
  it('masks every representation, once, before the request is made', async () => {
    const secrets: string[] = []
    const { impl } = recordingFetch([
      { status: 302, location: 'https://tsm-2.example.com/a' },
      { status: 200 },
    ])
    await postJson(DEST, {}, '{}', createHostAuthorizer('tsm.example.com,tsm-2.example.com'), {
      fetchImpl: impl,
      env: { HTTPS_PROXY: 'http://bob:hunter2@proxy.internal:3128' },
      setSecret: (s) => secrets.push(s),
    })
    expect(secrets).toContain('hunter2')
    // Deduped across hops: one ::add-mask:: per distinct secret, not one per hop.
    expect(new Set(secrets).size).toBe(secrets.length)
  })
})

// ---------------------------------------------------------------------------
// Positive control: a REAL CONNECT tunnel actually carries the callback
// ---------------------------------------------------------------------------
...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), 'tdr-proxy-'))
  ...
  // A minimal forward proxy: it answers CONNECT by opening a TCP socket to the
  // requested destination and splicing the two together, which is exactly the
  // tunnel an enterprise egress proxy provides.
...
describe('the callback really travels through a configured proxy (real CONNECT tunnel)', () => {
  it('tunnels to the destination and returns its response', async () => {
    connects = []
    hits = 0
    const response = await postJson(
      `https://localhost:${tlsPort}/api/v1/drift/ingest`,
      { 'X-TSM-Callback-Token': 'secret' },
      '{"status":"completed"}',
      createHostAuthorizer('localhost'),
      {
        caCert: caPem,
    ...
    expect(response).toEqual({ status: 200, body: '{"ok":true}' })
    // The control: the request went THROUGH the chokepoint, and the tunnel was
    // opened to the destination — not to some other host.
    expect(connects).toEqual([`localhost:${tlsPort}`])
    expect(hits).toBe(1)
  }, 30_000)

  it('keeps verifying the destination certificate through the tunnel', async () => {
    // A tunnel must not become a way to lose peer verification: the private CA
    // is trusted for the DESTINATION handshake, so dropping the trust anchor
    // has to fail even though the proxy leg itself is plain HTTP.
    connects = []
    await expect(
      postJson(
        `https://localhost:${tlsPort}/api/v1/drift/ingest`,
    ...
    // more than once, because the shared client reads a refused handshake as a
    // transient transport failure and retries it. Every attempt must still have
    // been addressed to the destination.
